...
function DocumentsContent() {
  ...
  const getButtonClass = (isActive: boolean) => {
    if (isActive) {
      return 'w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-sm font-medium transition-all bg-jkap-navy-500/20 text-foreground border border-jkap-navy-500/30';
    }
    return 'w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-sm font-medium transition-all text-muted-foreground hover:text-foreground hover:bg-muted';
  };
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {doc.isNew && <Badge variant="delinquent" className="text-[10px]">NEW</Badge>}
                          </div>
                          <p className="text-muted-foreground text-sm mb-3">{doc.description}</p>
                          <div className="flex items-center gap-4 text-xs text-muted-foreground">
                            <Badge variant="default" className="text-[10px]" icon={config.icon}>{config.label}</Badge>
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <Button as="link" href={doc.url} variant="secondary" size="sm" icon={<Eye className="w-4 h-4" />}>View</Button>
                          {doc.type === 'pdf' && <Button as="link" href={doc.url} variant="primary" size="sm" icon={<Download className="w-4 h-4" />}>Download</Button>}
                          {doc.type === 'form' && <Button as="link" href={doc.url} variant="primary" size="sm" icon={<ExternalLink className="w-4 h-4" />}>Open Form</Button>}
                        </div>
                      </div>
                    </Card>
                  );
